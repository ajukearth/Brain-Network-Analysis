...
# Correlation/pearson.R - Pearson correlation implementation
#########################################################

#' Calculate Pearson correlation matrix for brain regions
#' 
...
#'   - fisher_transform: Whether to apply Fisher's z-transformation
#' @return Correlation matrix
#' @export
calculate_pearson_correlation <- function(data, settings = NULL) {
  ...
  fisher_transform <- ifelse(is.null(settings$fisher_transform), FALSE, settings$fisher_transform)
  ...
  correlation_matrix <- stats::cor(data, method = "pearson", use = use_method)
  
  # Apply Fisher's z-transformation if requested
  if (fisher_transform) {
    correlation_matrix <- atanh(correlation_matrix)
  }
  ...
  # Additional metadata
  attr(correlation_matrix, "method") <- "pearson"
